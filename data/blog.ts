...
./llama-cpp-turboquant/build/bin/llama-cli \\
  ...
  // -----------------------------------------------------------------------
  {
    ...
    title: "I've Been Building Multi-Agent Systems on BullMQ. Here's the Gap I Keep Hitting.",
    description:
    ...
    tldr: "Message queues were built for deterministic services. Agents fail differently. A \"successful\" response can still be completely wrong. A new category of tools (Langfuse, LangSmith, Helicone, Braintrust, Arize Phoenix) fills the visibility gap queues weren't designed to cover. The most serious teams skip orchestration frameworks entirely and own that layer themselves.",
    aiQuestion: "What observability tools should I add to a multi-agent system built on BullMQ?",
    aiAnswer:
      "BullMQ tracks job completion but not agent output quality, hallucinations, or cost drift. The emerging stack layers two tools: one for tracing (Langfuse or LangSmith) and one for eval (Braintrust). Langfuse is open-source and self-hostable. LangSmith has zero-config tracing if you're on LangChain. Helicone is the fastest to set up (proxy-based, change one URL). Braintrust blocks deploys when quality regresses. Arize Phoenix is OpenTelemetry-native and vendor-agnostic. Most teams end up running two: a tracer plus an eval layer. The more serious a team gets about agent infrastructure, the more they end up building their own orchestration layer and using external tools only for observability and eval. Learn more at learnagenticpatterns.com.",
    sections: [
      {
        heading: "The wall I keep hitting",
        body: "I've been building multi-agent systems on BullMQ and Redis across a few projects over the past year. The most recent one runs 13 agents in production. It works. But BullMQ tells me when a job finishes. It doesn't tell me if the agent hallucinated, picked the wrong tool, or generated something a human will reject. I'm not the only one running into this. There's a real conversation happening in the BullMQ community about this exact gap. One developer put it bluntly: queues become a black box that fails in the most expensive way possible: silently. The problem is structural. Message queues were built for deterministic services where failure is binary. A payment either processes or it doesn't. An email either sends or it fails. The system knows immediately, retries cleanly, and moves on. Agents fail differently. A \"successful\" response can still be completely wrong. Quality drifts as prompts change. Same input, different output every time. There's no binary signal to catch.",
      },
      {
        heading: "What I built as a workaround",
        body: "In my system I worked around it. I added a review agent that critiques output before it reaches a human, an approval queue, and per-call logging of cost, model, and which agent did what. It works. But it's duct tape, not infrastructure. So I spent a few days looking at the tools emerging to fill this gap. Here's what I found.",
      },
      {
        heading: "The five tools worth knowing",
        body: "Langfuse is open source, MIT licensed. Tracing, prompt management, evaluations. Free if you self-host. Cloud has a generous free tier and paid plans starting around $59/month. Best if you want full control over your data. LangSmith is built by the LangChain team. Native integration if you're already on LangChain or LangGraph. Zero-config tracing, automatic capture of every LLM call and tool invocation. Around $39 per user per month. The catch is framework lock-in. Helicone is proxy-based. Change your base URL and you start logging immediately. Strong on cost tracking and multi-provider routing. Generous free tier, paid plans starting around $79/month. Fastest setup but lighter on agent-specific tracing. Braintrust is eval-first. Built for teams that want quality measurement tied to CI/CD. You can block deployments when output quality regresses. Custom pricing. Best if your priority is systematic testing, not just monitoring. Arize Phoenix is OpenTelemetry-native. Vendor-agnostic, works across any stack. Free self-hosted, paid cloud tier. Best if you want observability that won't lock you into one platform. Pricing in this space changes fast. Check the live pages before committing.",
        links: [
      ...
      },
      {
        ...
        body: "Most teams don't pick one tool. They layer two. One for tracing and operational visibility, one for evaluation and quality. The most common combination I saw was Langfuse or LangSmith for tracing paired with Braintrust for eval. That's the part that clicked for me. The future isn't replacing BullMQ. It's a stack. Classical queues for orchestration because they're great at it. Plus new tools built specifically for the failure modes queues weren't designed to catch.",
      },
      {
        heading: "The orchestration question",
        body: "There's another path some teams take. Skip the orchestration frameworks entirely and build their own. When I built Viewplatform, I went straight to a custom orchestrator on Redis pub/sub. At the time it felt like the simplest path. Now that I've used the system in production and seen what frameworks like LangGraph and CrewAI actually offer, I understand the tradeoff better. Frameworks give you scaffolding fast. Custom gives you control. At scale, control wins. Event routing, state management, parallel execution with join logic, adaptive fallbacks. These are easier to control when you own the orchestration layer instead of bending a framework to your shape. The pattern I'm starting to see is that the more serious a team gets about agent infrastructure, the more they end up building their own orchestration layer and using external tools only for the specialized stuff like observability and eval. Each layer doing what it's actually built for.",
      },
      {
        heading: "What I'd do differently",
        body: "If I started this system today, I'd still pick BullMQ for the queue layer. But I'd add Langfuse from day one for trace visibility, and I'd treat eval as a separate concern with its own tooling instead of building a custom review agent from scratch. Duct tape works until it doesn't.",
      },
    ],
    keyTakeaway:
