...
import { communityPartnerSchema } from "@/lib/validations";
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = communityPartnerSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const { name, email, channelUrl, platform, whyPartner } = parsed.data;

    const dailyKey = `community-partner:${ip}`;
    ...
    await query(
      `INSERT INTO community_partner_applications (name, email, channel_url, platform, why_partner)
       VALUES ($1, $2, $3, $4, $5)`,
      [name, email, channelUrl, platform, whyPartner]
    );
    ...
    if (resendKey && adminEmail) {
      ...
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: fromEmail,
          to: adminEmail,
          subject: `Community Partner Application: ${esc(name)} (${platform})`,
          html: `
            <div style="font-family:monospace;max-width:560px;margin:0 auto;padding:24px">
              <h2 style="color:#00D4FF;margin-bottom:16px">New Community Partner Application</h2>
              <div style="background:#0F1629;border:1px solid #1E293B;border-radius:8px;padding:20px">
                <table style="width:100%;border-collapse:collapse;color:#E2E8F0;font-size:14px">
                  ...
                  <tr>
                    ...
                    <td style="padding:8px 0;white-space:pre-wrap">${esc(whyPartner)}</td>
                  </tr>
                </table>
              </div>
              <p style="color:#64748B;font-size:12px;margin-top:16px">
                Submitted from the Community Partner page on learnagenticpatterns.com
              </p>
            </div>
          `,
        }),
    ...
    console.error("Community Partner API error:", error);
    return NextResponse.json(
